...
export default async function EditDataPage({ params }: EditDataPageProps) {
  ...
  const initialData = {
    ...
    collectedAt: data.collected_at ? new Date(new Date(data.collected_at).getTime() - 9 * 60 * 60 * 1000) : undefined,
    publishedAt: data.published_date ? new Date(new Date(data.published_date).getTime() - 9 * 60 * 60 * 1000) : undefined,
    title: data.title || "",
